...
class MonoDEst(nn.Module):
    ...
    def infer_wrapper(self, rgbs, scale=None, shift=None):
        """
        A wrapper for the infer method that adds additional checks and functionality
        """
        # 检查 self.model 是否有 infer 方法
        if hasattr(self.model, 'infer'):
            # 如果有，使用原有的 infer 方法
            return self.infer(rgbs, scale, shift)
        else:
            # 如果没有，假设它是一个标准的 PyTorch 模型
            depth_map = self.model(rgbs)

            # 如果是 depthAny 模型，进行特殊处理
            if self.mde_name == "depthAny":
                metric_dp = self.metric3d.infer(rgbs[:20])
                metric_dp_inv = 1 / metric_dp
                dp_0_rel = depth_map[:20]
    ...
    def forward(self, rgbs, scale=None, shift=None):
        """
        Forward pass, 调用 infer_wrapper 方法
        """
        return self.infer_wrapper(rgbs, scale, shift)
